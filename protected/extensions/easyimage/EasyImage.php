...
class EasyImage extends CApplicationComponent
{
    ...
    public function crop($width2, $height2, $position)
    {
        $newSize = $this->getNewImgSize($this->_image->width, $this->_image->height, $width2, $height2);
        $posotionArray = $this->getCoords($this->_image->width, $this->_image->height, $newSize['height'], $newSize['width'], $position );
        return $this->image()->crop($newSize['width'], $newSize['height'], $posotionArray['x'], $posotionArray['y']);

    }
    
    function getNewImgSize($height1 = null,  $width1 = null, $width2, $height2){
        if (!$width1 && !$width2) return false;
            $newheight = floor($height1*$height2/$width2);
        if($newheight <= $width1) return array("width" => $height1, "height" => $newheight);
            $newwidth = floor($width1*$width2/$height2);
        if($newwidth <= $height1) return array("width" => $newwidth, "height" => $width1);
    }      
    function getCoords($width1,$height1,$height2,$width2,$position){
            ...
            switch ($y_align){
                    case "top": $return['y'] = 0; 
                            break;
                    case "center": $return['y'] = floor(($height1 - $height2) / 2); 
                            break;	
                    case "bottom": $return['y'] = $height1 - $height2;
                            break;				
            }
            switch ($x_align){
                    case "left": $return['x'] = 0;
                            break;
                    case "center": $return['x'] = floor(($width1 - $width2) / 2);
                            break;	
                    case "right": $return['x'] = $width1 - $width2; 
                            break;				
            }
